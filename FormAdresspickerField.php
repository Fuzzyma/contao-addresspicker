...
class FormAdresspickerField extends FormTextField
{
    ...
    public function generate()
    {
        ...
        return sprintf('<input type="text" name="%s" id="ctrl_%s" class="text%s"%s onfocus="Backend.getScrollOffset()">%s%s',
                        $this->strName,
                        $this->strId,
    ...
    public function getScriptTag(){
        ...
        $b = null;
        if($this->bounds){
            $latLong = explode(';',$this->bounds);
            $b = [explode(';',trim($latLong[0])), explode(';',trim($latLong[1]))];
        }

        $c = $this->category;
        $r = $this->componentRestrictions;
